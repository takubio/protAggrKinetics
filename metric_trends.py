...
parser.add_argument('--xlimit', nargs = '?', required=False, type = float, help = "Limit for X-axis for plotting");
...
parser.add_argument('--flsf', nargs = '?', default=0, type = float, help = "Fraction of low signal filtering. If the signal for one of the concentration is smaller than [flsf]*[highest signal among all the replicates], then this low signal will be not normalized and set to zero");
args = parser.parse_args();

# Define numerictools functions
...
def get_statistics(xvalues, yvalues, window, minfraction):
    ...
    initial_growth_speed = np.mean(derivatives[:20])
    ...
    return xvalues[growth_start+window], xvalues[growth_max_index + window], xvalues[growth_end + window], growth_max_value, growth_total_speed, plateau_value, acceleration_max, initial_growth_speed
...
def process_sample_with_replicates(data, times, window, minfraction):
    ...
    local_list = [];
    for yvalues in data: 
        growth_start, growth_max_index, growth_end, growth_max_value, growth_total_speed, plateau_value, acceleration_max, initial_growth_speed = get_statistics(times, yvalues, window, minfraction);
        
        local_list.append(( growth_start, growth_max_index-growth_start, growth_end-growth_start, growth_total_speed, growth_max_value, plateau_value, (growth_max_index-growth_start)/(growth_end-growth_start), acceleration_max, initial_growth_speed ))
...
def draw_technical(times, merged, window, sample, channel, format_, ylimit, outdir, data_limit=400):
    ...
    growth_start, growth_max_index, growth_end, growth_max_value, growth_total_speed, plateau_value, acceleration_max, initial_growth_speed = get_statistics(times, merged, window, args.minfraction)
    if(data_limit):
...
for channel, lines in enumerate(channel2lines.values(), start=1):
    ...
    for sample, concentration in sample2concentration:
        ...
        full_data_list.append((sample, concentration, data))
           
    if(args.norm):
        temp = [];
        signal_maxes = [max([max(y) for y in x[2]]) for x in full_data_list]
        limit = args.flsf*max(signal_maxes)
        for (sample, concentration, data), smax in zip(full_data_list, signal_maxes):
            ndata = [np.array(x)-min(x) for x in data]
            ndata = [x/max(x) for x in ndata]
            if(smax <= limit):
                ndata = [x/100 for x in ndata]
                sys.stderr.write('Sample "%s" for concetration "%s" was not normalized since it\'s max signal intensity (%1.1f) is below the limit %1.1f\n\n' % (sample, concentration, smax, limit))
            temp.append((sample, concentration, ndata))
        full_data_list = temp

        
    for sample, concentration, data in full_data_list:
...
stypes = ['growth_start', 'growth_acceleration_duration', 'growth_length_duration', 'growth_average_speed', 'growth_max_speed', 'plateau', 'acceleration_fraction', 'acceleration_max', 'initial_growth_speed']
ylabels = ['time [h]', 'time [h]', 'time [h]', 'growth speed [h^-1]', 'maximal growth speed [h^-1]', 'plateau intensity [a.u]', 'ratio', 'maximal acceleration [h^-2]', 'growth speed [h^-1]']
...
for channel, statistic_list in statistic_dict.items():
    ...
    for stype, ylabel, statistic, errors in zip(stypes, ylabels, statistic_list, error_list):
        ...
        ax1.set_xlabel('PFD relative molar concentration', fontsize = FONTSIZE)
